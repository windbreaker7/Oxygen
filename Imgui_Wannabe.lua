...
-- Helper: Attach local workspace background image directly to the primary Iris window frame
local function attachIrisWindowBackground(fileName, opacity)
    if isfile and not isfile(fileName) then
        return
    end

    local guiContainer = targetParent:FindFirstChild("Oxygen_Iris_UI") or targetParent:FindFirstChild("Iris") or playerGui:FindFirstChild("Iris")
    if not guiContainer then return end

    -- Locate the top-level window frame generated by Iris
    local windowFrame = guiContainer:FindFirstChildWhichIsA("Frame", true)
    if not windowFrame then return end

    if windowFrame:FindFirstChild("IrisDeltaBG") then return end
...
Iris:Connect(makeCClosure(function()
    ...
    -- Inject background image to the inner window frame
    attachIrisWindowBackground("background.jpeg", 0.30)
